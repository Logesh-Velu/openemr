...
function calcTaxes($row, $amount)
{
    ...
    foreach ($arates as $value) {
        ...
        $trow = sqlQuery("SELECT option_value FROM list_options WHERE " . "list_id = 'taxrate' AND option_id = ? LIMIT 1", array(
            $value
        ));
...
$patdata = sqlQuery("SELECT " . "p.fname, p.mname, p.lname, p.postal_code, p.pubpid,p.pid, i.copay " . "FROM patient_data AS p " . "LEFT OUTER JOIN insurance_data AS i ON " . "i.pid = p.pid AND i.type = 'primary' " . "WHERE p.pid = ? ORDER BY i.date DESC LIMIT 1", array(
    $pid
));
...
if (($_POST['form_save'] ?? null) || ($_REQUEST['receipt'] ?? null)) {
    ...
    $payrow = sqlQuery("SELECT " . "SUM(amount1) AS amount1, " . "SUM(amount2) AS amount2, " . "MAX(method) AS method, " . "MAX(source) AS source, " . "MAX(dtime) AS dtime, " .
        // "MAX(user) AS user " .
        "MAX(user) AS user, " . "MAX(encounter) as encounter " . "FROM payments WHERE " . "pid = ? AND dtime = ?", array(
        $form_pid,
        $timestamp
    ));
    ...
    $frow = sqlQuery("SELECT * FROM facility " . " WHERE id = ?", array(
        $tmprow['facility_id']
    ));
                ...
                <tbody>
                    ...
                    foreach ($encs as $key => $value) {
                        ...
                        if ((($NumberOfInsurance == 0 || $value['last_level_closed'] == 4 || $NumberOfInsurance == $value['last_level_closed']))) { // Patient balance
                            $brow = sqlQuery("SELECT SUM(fee) AS amount FROM billing WHERE " . "pid = ? and encounter = ? AND activity = 1", array(
                                $pid,
                                $enc
                            ));
                            $srow = sqlQuery("SELECT SUM(fee) AS amount FROM drug_sales WHERE " . "pid = ? and encounter = ? ", array(
                                $pid,
                                $enc
                            ));
                            $drow = sqlQuery(
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    <h4 style="display: inline-block;"><?php echo xlt('Payment Amount'); ?>:&nbsp;
                                                        <span class="font-weight-bold"><span id="payTotal"></span></span>
                                                    </h4>
                                                </div>
                                            </div>
                                        </div>
        ...
        ?>
            <script>
                ...
                function stripeTokenHandler(token) {
                    ...
                    let inv_values = JSON.stringify(getFormObj('invoiceForm'));
                    document.getElementById("invValues").value = inv_values;
                    const autopaymentCheckbox = document.getElementById('autopayment');
                    ...
                    fetch('./lib/paylib.php', {
                        ...
                        if (autopaymentCheckbox.checked) {
                            const pid = "<?php echo $_SESSION['pid']; ?>";
                            const cardholderName = document.getElementById('cardholder-name')?.value || '';
                            let customerId = ''; // ✅ store customer_id for later

                            // Debug log
                            console.log('Starting auto-payment setup process');
                            ...
                            stripe.createPaymentMethod({
                                ...
                                console.log('Payment method created:', paymentMethodResult.paymentMethod.id);

                                // 2. Create customer and attach payment method
                                return fetch('./lib/create-stripe-customer.php', {
                                    method: 'POST',
                                    headers: {
                                ...
                                customerId = customerData.customer_id; // ✅ store it here
                                console.log('Customer created:', customerId);

                                // 3. Create setup intent for future payments
                                return fetch(`./lib/create-setup-intent.php?customer_id=${customerId}`);
                            }).then(res => {
                                if (!res.ok) throw new Error('Setup intent creation failed');
                                return res.json();
                            }).then(setupIntentData => {
                                if (setupIntentData.error) {
                                    throw new Error(setupIntentData.error);
                                }

                                console.log('Setup intent created:', setupIntentData.clientSecret);

                                // 4. Confirm the setup intent
                                return stripe.confirmCardSetup(setupIntentData.clientSecret, {
                                    payment_method: {
                                        card: card,
                                        billing_details: {
                                            name: cardholderName
                                ...
                                console.log('Setup intent confirmed:', confirmResult.setupIntent.payment_method);

                                // 5. Save payment method to your database
                                return fetch('./lib/save-payment-method.php', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json'
                                    },
                                    body: JSON.stringify({
                                        ...
                                        customer_id: customerId // ✅ use correct value
                                    })
                                });
                            }).then(saveResponse => {
                                if (!saveResponse.ok) {
                                ...
                                console.log('Payment method saved:', saveData);
                                alert('Card saved successfully for auto payment!');
                                window.location.reload();
                            }).catch(error => {
                                console.error('Error in auto-payment setup:', error);
                                alert('Error saving card for auto-payment: ' + error.message);
                            });
                        } else {
                            alert(chargeMsg);
                            window.location.reload(false);
